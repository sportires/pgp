...
class Interceptor extends \StripeIntegration\Payments\Setup\Migrate\AttributesCommand implements \Magento\Framework\Interception\InterceptorInterface
{
    ...
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'addAttributeToAllAttributeSets');
        if (!$pluginInfo) {
            return parent::addAttributeToAllAttributeSets($attributeCode, $attributeGroupName, $output);
        } else {
            return $this->___callPlugins('addAttributeToAllAttributeSets', func_get_args(), $pluginInfo);
        }
    }
