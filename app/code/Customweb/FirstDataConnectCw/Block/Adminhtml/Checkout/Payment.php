...
namespace Customweb\FirstDataConnectCw\Block\Adminhtml\Checkout;
...
class Payment extends \Magento\Backend\Block\Widget\Form\Container
{
	 ...
	 * @var string
	 */
	protected $_blockGroup = 'Customweb_FirstDataConnectCw';
	...
	protected function _construct()
	{
		...
		$this->setId('firstdataconnectcw_checkout_payment');

		$this->buttonList->update('save', 'label', __('Submit Order'));
		$this->buttonList->update('save', 'class', 'primary');
		$this->buttonList->update('save', 'id', 'firstdataconnectcw-payment-submit-button');
		...
		$this->buttonList->add(
			...
			[
				'label' => __('Cancel'),
				'class' => 'cancel',
				'id' => 'firstdataconnectcw-payment-cancel-button'
			]
		);
	}
	...
	public function getTransaction()
	{
		if (!($this->transaction instanceof \Customweb\FirstDataConnectCw\Model\Authorization\Transaction)) {
			...
			if (!($transaction instanceof \Customweb\FirstDataConnectCw\Model\Authorization\Transaction) || !$transaction->getId()) {
