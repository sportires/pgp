...
class RequireJsManager
{
    ...
    public function getRequireJsContent($routeKey)
    {
        ...
        $config = [
            'jsbuild' => $this->convertToUTF8($this->getInlineConfig($routeKey, '.js', $jsCallback)),
            'text' => $this->convertToUTF8($this->getInlineConfig($routeKey, '.html', $htmlCallback))
        ];
    ...
    protected function convertToUTF8($value)
    {
        if (is_array($value)) {
            foreach ($value as $key => $item) {
                $value[$key] = $this->convertToUTF8($item);
            }
