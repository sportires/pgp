                ...
                if ($product->getTypeId() == 'configurable') {
                    ...
                    if (isset($images) && $images->getSize() > 0) {
                        ...
                        foreach ($images as $key => $image) {
                            $tmp[$image->getData("position")] = $image;
                        }
                        ksort($tmp);
                        foreach ($tmp as $key => $image) {
                            ...
                            $i->setData($product->getSku(), $imageTypes[$counter++], $image->getUrl());
                            $envelope->addImage($i);
                        }
                    }
                    ...
                    foreach ($childs as $child) {
                        ...
                        if (isset($images) and $images->getSize() > 0) {
                            $counter = 0;
                            foreach ($images as $image) {
                                 ...
                                 */
                                $i = $this->image->create();
                                $i->setId($profile->getId() . $child->getId());
                                $i->setData($child->getSku(), $imageTypes[$counter++], $image->getUrl());
                                $envelope->addImage($i);
                            }
                        }
                    }
                    ...
                    if (isset($images) && $images->getSize() > 0) {
                        ...
                        foreach ($tmp as $key => $image) {
                             ...
                             */
                            $i = $this->image->create();
                            $i->setId($profile->getId() . $product->getId());
                            $i->setData($product->getSku(), $imageTypes[$counter++], $image->getUrl());
                            $envelope->addImage($i);
                        }
                    }
