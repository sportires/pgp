                        ...
                        if ($product->getStatus() == '1' || $this->config->createBackorder()) {
                            ...
                            if ($stockStatus) {
                                if ($product->getTypeId() == \Magento\Catalog\Model\Product\Type::TYPE_BUNDLE) {
                                    $discount[$product->getId()] = (float)$items->getPromotionDiscount($index, true);
                                    $discountAmount += $discount[$product->getId()];
                                    $shippingDiscount += (float)$items->getShippingDiscount($index, true);
                                } else {
            ...
            if ($itemAccepted > 0 && count($qouteItems) >= ($itemOrdered + $bundleItems)) {
                if (!empty($address)) {
                    ...
                    $shipAddress = [
                        ...
                        'street' => [
                            $this->getValue('AddressLine1', $address, ''),
