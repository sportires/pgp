...
class Upload extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if (count($ids) <= $chunkSize) {
            ...
            if ($response['success'] == 1) {
                $this->messageManager->addSuccessMessage(
                    count($ids) . ' Product(s) Uploaded/Updated successfully...'
                );
            } else {
                $message = 'Product(s) upload failed.';
                if (isset($response['message'])) {
                    $message = "Product(s) upload/update failed..See Errors";
                }
        ...
        $key = uniqid(\Ced\Claro\Helper\Config::ACTION_PRODUCT_UPLOAD . '_');
        ...
        $this->registry->register(
            ...
            \Ced\Claro\Helper\Config::ACTION_PRODUCT_UPLOAD
        );
