...
namespace Ced\Claro\Block\Adminhtml\Order\Button;

/**
 * Class Import
 * @package Ced\Claro\Block\Adminhtml\Product\Button
 */
    ...
    \Magento\Framework\View\Element\UiComponent\Control\ButtonProviderInterface
{
    ...
    public function buttonOptions()
    {
        ...
        $splitButtonOptions['order_import_pending'] = [
            'label' => __('Import Paid'),
            'onclick' => "setLocation('" . $this->getUrl('claro/order/import') . "')",
            'default' => false,
        ];

        $splitButtonOptions['order_search_import'] = [
            'label' => __('Search and Import'),
            'onclick' => "setLocation('" . $this->getUrl('claro/order/searchimport') . "')",
            'default' => true,
        ];
