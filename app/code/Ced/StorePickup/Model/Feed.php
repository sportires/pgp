...
class Feed extends \Magento\AdminNotification\Model\Feed
{
    ...
    public function __construct(
        ...
        \Ced\StorePickup\Helper\Data $storeHelper,
        array $data = []
    ){
    ...
    public function checkUpdate()
    {
        $feedData = array();
        $feed = array();
        $feedXml = $this->getFeedData($this->_objectManager->get('Ced\StorePickup\Helper\Feed')->getEnvironmentInformation());
        $allowedFeedType = explode(',', $this->_backendConfig->getValue(self::XML_FEED_TYPES));
        if ($feedXml && $feedXml->channel && $feedXml->channel->item) {
            $installedModules=$this->_objectManager->get('Ced\StorePickup\Helper\Feed')->getCedCommerceExtensions();
            foreach ($feedXml->channel->item as $item) {
                if(!isset($installedModules[(string)$item->module])) {
                    continue;
                }
                if($this->_objectManager->get('Ced\StorePickup\Helper\Feed')->isAllowedFeedType($item)) {
