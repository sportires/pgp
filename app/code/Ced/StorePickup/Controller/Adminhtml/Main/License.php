...
namespace Ced\StorePickup\Controller\Adminhtml\Main;
...
class License extends \Magento\Backend\App\Action
{
    ...
    public function __construct(
        ...
        $this->_storeHelper = $this->_objectManager->get('Ced\StorePickup\Helper\Feed');
    }
        ...
        if($postData){
            ...
            if ($response && isset($response['hash']) && isset($response['level'])) {
                ...
                try {
                    ...
                    {
                        $path = \Ced\StorePickup\Block\Extensions::HASH_PATH_PREFIX.strtolower($postData['module_name']).'_hash';
                        $this->_feedHelper->setDefaultStoreConfig($path, $response['hash'], 0);
                        $path = \Ced\StorePickup\Block\Extensions::HASH_PATH_PREFIX.strtolower($postData['module_name']).'_level';
