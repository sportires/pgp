...
class BeforeLoadLayout implements ObserverInterface
{
    ...
    public function __construct(
        ...
        $this->_feedHelper = $this->_objectManager->get('Ced\StorePickup\Helper\Feed');
        $this->_storeHelper = $this->_objectManager->get('Ced\StorePickup\Helper\Data');
    }
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
       ...
       try {
            ...
            $validateArray=array();
            if($controllerModule !== 'magento_config' || $request->getParam('section') !== 'carriers'){ return $this; } 
            $helper = $this->_objectManager->create('Ced\StorePickup\Helper\Feed');
            ...
            foreach ($modules as $moduleName=>$releaseVersion)
            {
                ...
                $h = $this->_objectManager->create('Ced\StorePickup\Helper\Data')->getStoreConfig(\Ced\StorePickup\Block\Extensions::HASH_PATH_PREFIX.$m.'_hash'); 
         
                for($i=1;$i<=(int)$this->_objectManager->create('Ced\StorePickup\Helper\Data')->getStoreConfig(\Ced\StorePickup\Block\Extensions::HASH_PATH_PREFIX.$m.'_level');$i++)
                    {  
                        $h = base64_decode($h);
                    }

                $h = json_decode($h,true);
                if(is_array($h) && isset($h['domain']) && isset($h['module_name']) && isset($h['license']) && strtolower($h['module_name']) == $m && $h['license'] == $this->_objectManager->create('Ced\StorePickup\Helper\Data')->getStoreConfig(\Ced\StorePickup\Block\Extensions::HASH_PATH_PREFIX.$m))
                 {}else{
    ...
    private function autoValidateModules($urlParams = array())
    {
        ...
        if (is_array($urlParams) && count($urlParams) > 0) {
            $body = $this->_storeHelper->addParams('',$urlParams);
            $body = trim($body,'?');
            ...
            foreach ($result as $moduleName => $response) {
                if ($response && isset($response['hash']) && isset($response['level'])) {
                    ...
                    try {
                        ...
                        {
                            $path = \Ced\StorePickup\Block\Extensions::HASH_PATH_PREFIX.strtolower($moduleName).'_hash';
                              $this->_feedHelper->setDefaultStoreConfig($path, $response['hash'], 0);
                            $path = \Ced\StorePickup\Block\Extensions::HASH_PATH_PREFIX.strtolower($moduleName).'_level';
                             $this->_feedHelper->setDefaultStoreConfig($path, $response['level'], 0);
                            $path = \Ced\StorePickup\Block\Extensions::HASH_PATH_PREFIX.strtolower($moduleName);
    ...
    private function getLicenseActivateUrl()
    {
        if (is_null($this->_licenseActivateUrl)) {
            $this->_licenseActivateUrl = 'https://'. $this->_feedHelper->getStoreConfig(self::LICENSE_ACTIVATION_URL_PATH);
        }
