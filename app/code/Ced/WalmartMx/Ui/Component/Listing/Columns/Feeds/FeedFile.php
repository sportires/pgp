...
class FeedFile extends Column
{
     ...
     * Url path 
     */
    const URL_PATH_RESEND = 'walmartmx/feeds/resend';
    ...
    public function getFileLink($filePath)
    {
        ...
        if (isset($filePath) and !empty($filePath)) {
            ...
            $file = $this->dl->getPath('media') . "/walmartmx/feed/" . $fileName;
            if ($this->file->fileExists($file)) {
                $url = $this->urlBuilder->getBaseUrl() . "pub/media/walmartmx/feed/" . $fileName;
            } else {
                if ($this->file->fileExists($filePath)) {
                    $cpDir = $this->dl->getPath('media') . "/walmartmx/feed/";
