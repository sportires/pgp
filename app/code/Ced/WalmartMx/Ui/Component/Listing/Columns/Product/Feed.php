...
namespace Ced\WalmartMx\Ui\Component\Listing\Columns\Product;
    ...
    public function prepareDataSource(array $dataSource)
    {
        if (isset($dataSource['data']['items'])) {
            ...
            foreach ($dataSource['data']['items'] as &$item) {
                ...
                $status = \Ced\WalmartMx\Model\Source\Product\Status::NOT_UPLOADED;
                if (isset($item[$fieldName])) {
                    if (isset($item['walmartmx_product_status']) and !empty($item['walmartmx_product_status'])) {
                        $status = $item['walmartmx_product_status'];
                        if ($status == \Ced\WalmartMx\Model\Source\Product\Status::INVALID) {
                            $class = 'grid-severity-minor';
                        }
                    }

                    $item[$fieldName . '_html'] = "<button style='width:100%' class='{$class}'>"."<span>{$status}</span></button>";
                    $item[$fieldName . '_title'] = __('WalmartMx Feed Details');
                    $item[$fieldName . '_productid'] = $item['entity_id'];
                    if (isset($item['walmartmx_feed_errors'])) {
                        $item[$fieldName . '_product_feed_errors'] = $item['walmartmx_feed_errors'];
                    } else {
