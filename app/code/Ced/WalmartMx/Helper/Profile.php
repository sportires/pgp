...
/**
 * Class Profile For WalmartMx Profiling
 *
...
class Profile extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    */
    ...
    public function __construct(
        ...
        \Ced\WalmartMx\Model\ProfileFactory $profile,
        \Ced\WalmartMx\Helper\Cache $cache
    ) {
        $this->json = $json;
        $this->profileProduct = $profileProduct;
        $this->walmartmxCache = $cache;
     ...
     * @return \Ced\WalmartMx\Helper\Profile|null
     */
    public function getProfile($productId = null, $profileId = null)
    {
        if (empty($profileId) or !is_numeric($profileId)) {
            $profileId = $this->profileProduct->load($productId, 'product_id')
                ->getWalmartmxProfileId();
        }

        $this->profile = $this->walmartmxCache->getValue(\Ced\WalmartMx\Helper\Cache::PROFILE_CACHE_KEY . $profileId);
    ...
    public function setProfile($profileId = null)
    {
        if (isset($profileId)) {
            ...
            if (isset($this->profile) and is_array($this->profile)) {
                ...
                $this->walmartmxCache->setValue(\Ced\WalmartMx\Helper\Cache::PROFILE_CACHE_KEY . $profileId, $this->profile);
                return true;
            }
        }
        return false;
