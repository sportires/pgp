...
class MassDelete extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if (!is_array($profileIdsToDelete) && !$excluded) {
            ...
            $profileIdsToDelete  = $this->_objectManager->create('Ced\WalmartMx\Model\Profile')
        ...
        if (!empty($profileIdsToDelete)) {
            try {
                foreach ($profileIdsToDelete as $profileId) {
                    $profile = $this->_objectManager->create('Ced\WalmartMx\Model\Profile')->load($profileId);
                    $profile->delete();
                }
