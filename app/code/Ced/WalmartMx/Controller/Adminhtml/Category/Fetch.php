...
namespace Ced\WalmartMx\Controller\Adminhtml\Category;
...
class Fetch extends Action
{
    ...
    public function execute()
    {
        ...
        $loadCatObj = $this->_objectManager->create('Ced\WalmartMx\Model\Profile')->load($current_profile_id);
        $check =array();
            ...
            foreach ($response as $value) {
                if (isset($value['level']) && ($value["level"] >= $level)) {
                    ...
                    if (isset($check['select-level'.$level]) && $value["code"] == $check['select-level'.$level]) {
                        $categoryHtml .= '<option selected="selected" value="'.$value["code"].'">'.$value["label_translations"]['label_translation']['value'].'</option>';
                    } else {
                        $categoryHtml .= '<option value="'.$value["code"].'">'.$value["label_translations"]['label_translation']['value'].'</option>';
                    }
                }
            }
