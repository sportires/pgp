...
class Sync extends Action
{
    ...
    public function __construct(
        ...
        \WalmartMxSdk\ProductFactory $feedrequest,
        ...
        $this->walmartmxProducthelper = $product;
    ...
    public function execute()
    {
        ...
        if ($feed and $feed->getId()) {
            ...
            if($feed->getType()=='inventory-update')
                $feed_data = $this->feedrequest->create(['config' => $this->config->getApiConfig()])->getFeeds($feed->getFeedId() ,\WalmartMxSdk\Core\Request::POST_OFFER_IMPORT.'/'.$feed->getFeedId());
            else
                $feed_data = $this->feedrequest->create(['config' => $this->config->getApiConfig()])->getFeeds($feed->getFeedId() ,\WalmartMxSdk\Core\Request::POST_ITEMS_SUB_URL.'/'.$feed->getFeedId());

            $feedError = $this->walmartmxProducthelper->getFeeds($feed->getFeedId(), $feed_data);
