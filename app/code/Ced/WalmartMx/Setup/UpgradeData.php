...
class UpgradeData implements UpgradeDataInterface
{
    ...
    public function upgrade(ModuleDataSetupInterface $setup, ModuleContextInterface $context)
    {
        ...
        if (version_compare($context->getVersion(), '0.0.2', '<')) {
            ...
            if (!$this->eavAttribute->getIdByCode('catalog_product', 'walmartmx_profile_id')) {
                $eavSetup->addAttribute(
                    'catalog_product',
                    'walmartmx_profile_id',
                    [
                        'group' => $groupName,
                        'note' => 'WalmartMx Profile Id',
                        'input' => 'text',
                        'type' => 'varchar',
                        'label' => 'WalmartMx Profile Id ',
                        'backend' => '',
                        'visible' => 1,
                        'required' => 0,
                        'sort_order' => 1,
            ...
            if (!$this->eavAttribute->getIdByCode('catalog_product', 'catch_exclude_from_sync')) {
                $eavSetup->addAttribute(
                    'catalog_product',
                    'catch_exclude_from_sync',
                    [
                        'group' => $groupName,
                        'note' => 'If yes then product syncing will not done for this product',
                        'input' => 'select',
                        'source' => 'Ced\WalmartMx\Model\Source\Yesno',
                        'type' => 'varchar',
