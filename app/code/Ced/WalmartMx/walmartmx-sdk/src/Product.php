...
class Product extends \WalmartMxSdk\Core\Request
{

    public function getAuthorisation()
    {
        $response = $this->getRequest(self::GET_ACCOUNT_SUB_URL);
        $response = $this->xmlToArray($response);
        if (isset($response['shop']) && !empty($response['shop'])) {
            return true;
        }
        return false;
    }
    ...
    public function getCategories($params, $forceFetch = false)
    {
        ...
        try {
            ...
            if ($this->debugMode) {
                $this->logger->debug(
                    "WalmartMxSdk\\Product\\getCategories() : Errors: " . var_export($e->getMessage(), true)
                );
            }
        }
        return $categories;
...
//                    print_r($attribute);die;
                    if($attribute['type'] == "LIST" || $attribute['type'] == "LIST_MULTIPLE_VALUES") {
                        if ($attribute['code'] == 'AMM_MARCA') {
                            $allowed = ['Michelin','BF Goodrich', 'FIRESTONE', 'Winda', 'Goodyear', 'Continental', 'Pirelli', 'Bridgestone', 'KHUMO'];
                            $mergingBrands = [];
...
//                    [value]';
                            ...
                            $valueLists['values_list']['values']['value'] = array_slice($valueLists['values_list']['values']['value'], 0, 400);
//                            echo '<pre>';
//                            print_r($valueLists);die;
                        }
            ...
            if ($this->debugMode) {
                $this->logger->debug(
                    "WalmartMxSdk\\Product\\getAttributes() : Errors: " . var_export($e->getMessage(), true)
                );
            }
        }
        return $attributes;
	...
	/**
     * Create/Update Offer on WalmartMx
     * @param $data
     * @return bool|array
     */
    ...
    /**
     * Deactivate Product on WalmartMx
     * @param $data
     * @return bool | array
     */
    ...
    public function updateInventory($data)
    {
        if (isset($data[0]['item'])) {
            ...
            $xml = new \WalmartMxSdk\Generator();
    ...
    // Get inventory for all products uploaded on catch for status change
    public function getInventory($skus = [])
    {
        if (!empty($skus) and is_array($skus)) {
            ...
            try {
                ...
                if ($this->debugMode) {
                    $this->logger->addDebug('\WalmartMxSdk\Product\getInventory(): Error: ' . $e->getMessage());
                }
                return false;
            }
        }
    ...
    public function getValueLists($attributeCode, $forceFetch = false, $url = self::GET_ATTRIBUTES_VALUE_LIST)
    {
        ...
        try {
            ...
            if ($this->debugMode) {
                $this->logger->debug(
                    "WalmartMxSdk\\Product\\getValueLists() : Errors: " . var_export($e->getMessage(), true)
                );
            }
            return $valueLists;
        }
    ...
    public function getOffers($chunkSize = 50, $offset = 0, $url = self::GET_OFFERS)
    {
        ...
        try {
            ...
            if ($this->debugMode) {
                $this->logger->debug(
                    "WalmartMxSdk\\Product\\getOffers() : Errors: " . var_export($e->getMessage(), true)
                );
            }
            return $offers;
        }
